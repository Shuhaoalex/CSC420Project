...
dataset = Dataset(dataset_config, dataset_config["img_train_flist"], dataset_config["mask_train_flist"])
model = InpaitingModel(model_config)

batch_size = dataset_config["batch_size"]

if model_config["edge"]["train"]:
    edge_dataset = dataset.get_edge_dataset().batch(batch_size).prefetch(10)
...
if model_config["clr"]["train"]:
    color_dataset = dataset.get_color_dataset().batch(batch_size).prefetch(10)
